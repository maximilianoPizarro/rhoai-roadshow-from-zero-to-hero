...
curl -v http://customer-service-mcp.neuralbank.svc.cluster.local:8080/api/credit-risk/query \
...
After applying the policy, you can test the rate limiting using `curl` commands. The following example shows how to test the MCP endpoint:

![Rate Limit API Test](../images/connectivity-link-policy-ratelimit-api-curl.png)
