...
1. **Navigate to Keycloak Console**:
   <a href="https://keycloak-keycloak.apps.<CLUSTER_DOMAIN>" target="_blank">https://keycloak-keycloak.apps.<CLUSTER_DOMAIN></a>
...
2. **Assign Realm Roles**: Add appropriate roles (e.g., `commercial-agent`, `developer`)
3. **Assign Client Roles**: If needed, assign client-specific roles
...
User attempts to access a protected Neuralbank service (e.g., Playground, Frontend)
